...
public class ProductController extends AbstractController<Product, ProductDto> {
	...
	@ResponseStatus(HttpStatus.CREATED)
	public ResponseEntity<ProductDto> add(final @PathVariable("productId") Integer productId,
	...
	public ResponseEntity<ProductDto> removeCategory(final @PathVariable("productId") Integer productId,
			final @PathVariable("categoryId") Integer categoryId) {
		return ResponseEntity.accepted().body(facade.removeCategory(categoryId, productId));
	}

	@PutMapping("/{productId}/remove/price/{priceId}")
	public ResponseEntity<ProductDto> removePrice(final @PathVariable("productId") Integer productId,
			final @PathVariable("priceId") Integer priceId) {
		return ResponseEntity.accepted().body(facade.removePrice(priceId, productId));
	}

	@GetMapping("/price")
	public ResponseEntity<List<ProductDto>> findAllByPrice(@Valid final @RequestBody PriceDto priceDto,
			final Pageable pageable) {
		return ResponseEntity.ok(facade.findAll(pageable, priceDto));
	}

	@GetMapping("/category/{categoryId}")
	public ResponseEntity<List<ProductDto>> findAllByCategory(final @PathVariable("categoryId") Integer categoryId,
			final Pageable pageable) {
		return ResponseEntity.ok(facade.findAll(pageable, categoryId));
	}

	@GetMapping("/name/{name}")
	public ResponseEntity<List<ProductDto>> findAllName(final @PathVariable("name") String name,
			final Pageable pageable) {
