...
public class PriceControllerTest {
		...
		final String message = mockMvc
				.perform(put(URL_API + "update").contentType(APPLICATION_JSON)
						.content(mapper.writeValueAsString(
								PriceDto.builder().currency(value).amount(BigDecimal.valueOf(100)).build())))
	...
	public void test_validation_PriceDto_amount() throws Exception {
		...
		final String message = mockMvc
				.perform(put(URL_API + "update").contentType(APPLICATION_JSON)
						.content(mapper.writeValueAsString(
								PriceDto.builder().currency(BYN).amount(BigDecimal.valueOf(value)).build())))
				.andExpect(status().isBadRequest()).andReturn().getResolvedException().getMessage();
	...
	public void test_findByCurrency() throws Exception {
		...
		mockMvc.perform(get(URL_API + "currency/{currency}", BYN)).andExpect(status().isOk())
	...
	public void test_findAllByProduct() throws Exception {
		...
		mockMvc.perform(get(URL_API + "product/{id}", 0)).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
				.andExpect(jsonPath("$.*", hasSize(0)));
	}
	...
	public void test_findAllByProduct_400() throws Exception {
		...
		final String message = mockMvc.perform(get(URL_API + "product/{id}", id)).andExpect(status().isBadRequest())
				.andReturn().getResolvedException().getMessage();
	...
	public void test_findAllByProductJson_400() throws Exception {
		...
		final String message = mockMvc
				.perform(get(URL_API + "product").contentType(APPLICATION_JSON)
	...
	public void test_findBetween() throws Exception {
		...
		Mockito.when(priceFacade.findAll(Mockito.any(Pageable.class), Mockito.anyString(),
				Mockito.any(BigDecimal.class), Mockito.any(BigDecimal.class))).thenReturn(new ArrayList<>());

		mockMvc.perform(get(URL_API + "between/{currency}/{from}/{to}", BYN, 0, 1)).andExpect(status().isOk())
				.andExpect(content().contentTypeCompatibleWith(APPLICATION_JSON))
				.andExpect(jsonPath("$.*", hasSize(0)));
	}
